...
export default function EditEmployee() {
  ...
  const [formData, setFormData] = useState({
    ...
    bankName: '', bankAccount: '', payrollId: null
  });
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      try {
        ...
        let formattedDoj = '';
        if (data.doj) {
           formattedDoj = new Date(data.doj).toISOString().split('T')[0];
        }
        ...
        setFormData({
          ...
          payPeriodFrom: data.pay_period_from ? new Date(data.pay_period_from).toISOString().split('T')[0] : '',
          payPeriodTo: data.pay_period_to ? new Date(data.pay_period_to).toISOString().split('T')[0] : '',
          paidDays: data.paid_days || '',
          grossBasicPay: data.paid_days ? ((data.basic_pay * 30) / data.paid_days).toFixed(2) : (data.basic_pay || ''),
          ...
          profTax: data.prof_tax || '',
          medInsurance: data.med_insurance || '',
          sodexo: data.sodexo || '',
          payrollId: data.payroll_id || null
        });
      } catch (error) {
        console.error("Failed to load employee details", error);
        setStatus("Failed to load employee details.");
      }
    };
    fetchEmployee();
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error);
      setStatus('Failed to update. Please ensure backend is running.');
    }
  };
  ...
  return (
    <Box sx={{ maxWidth: 900, mx: 'auto', p: 2 }}>
      <Paper elevation={3} sx={{ p: 4, borderRadius: 3 }}>
        <Typography variant="h5" sx={{ mb: 2, fontWeight: 'bold', color: 'primary.main' }}>Edit Employee</Typography>
        <Typography variant="body2" sx={{ mb: 3, color: 'text.secondary' }}>Modify details below and save.</Typography>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Box sx={{ display: 'flex', gap: 2, mt: 4 }}>
            ...
            <Button variant="outlined" color="secondary" size="large" onClick={() => navigate('/employees')} fullWidth>
              Cancel
            </Button>
          </Box>
        </form>
